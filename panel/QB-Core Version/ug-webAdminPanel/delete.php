<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        $license = $_GET["license"];
        do {
            $sql = "DELETE FROM players WHERE license = '$license';";
            $result = $connection -> query($sql);
            if (!$result) {
                $errorMsg = "[ug-webAdminPanel] ERROR: Error deleting in the 'players' table! Query Error: " . $connection -> error;
                break;
            }
            $successMsg = "[ug-webAdminPanel] SUCCESS: Deleted the player with License Identifier '$license'!";
            $license = "";
        } while (false);
    }
?>
